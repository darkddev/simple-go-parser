...
func WB_MakeUrl(path string, params WStrMap) string {
	values := url.Values{}
...
func WB_MakePageUrl(mainUrl string, page int) string {
	pageUrl := mainUrl
	if page != 1 {
...
func WB_ParseSearchTile(item WsrSearchtem) WsSearchTile {
...
func WB_ParseProduct(item WbrProduct, position int, baseUrl string) WbProduct {
...
func Walmart_BrowsePageScraper(jsonTag *goquery.Selection) WbResult {
	var raw WbrResult
	var result WbResult
	var data WbData
...
func Walmart_IsBrowsePage(jsonTag *goquery.Selection) bool {
	var page WRawResult
	json.Unmarshal([]byte(jsonTag.Text()), &page)
	return page.Page != "" && strings.Split(page.Page, "/")[1] == "browse"
}
